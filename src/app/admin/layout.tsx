...
import { LayoutDashboard, Users, Clock, CheckCircle, LogOut, Menu, X, Home, MessageSquare, Settings, UserCog } from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-white border-r border-gray-100 transition-all duration-300 z-50 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
        <div className="p-6 flex items-center justify-between">
          {isSidebarOpen ? (
            <Image src="/images/blogo.webp" alt="Logo" width={140} height={35} className="h-7 w-auto"  unoptimized/>
          ) : (
        ...
        <nav className="mt-8 px-4 space-y-2">
          ...
          <Link
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold text-sm transition-all ${pathname === '/admin/management-users' ? 'bg-[#EC5B13] text-white shadow-lg shadow-[#EC5B13]/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
          >
            <UserCog size={20} />
            {isSidebarOpen && <span>Luxevillaz Management Users</span>}
          </Link>
          <Link
          ...
          <Link
            href="/admin/settings"
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold text-sm transition-all ${pathname === '/admin/settings' ? 'bg-[#EC5B13] text-white shadow-lg shadow-[#EC5B13]/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
          >
            <Settings size={20} />
            {isSidebarOpen && <span>Tracking Settings</span>}
          </Link>
        </nav>
